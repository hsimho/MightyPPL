        ...
        if (phi->interval() == nullptr) {
            ...
            *****/
            ...
            for (auto i = 0; i < phi->num_pairs; ++i) {
                ...
                // 1 -> 1, !in_i & !out_i & *phi, x <= b, y <= a

                build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1", "1", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), 0, !in_i & !out_i & phi->atom(1)->star);

                // 1 -> 1, in_i & !out_i & *phi, x := 0, x <= b, y <= a

                build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1", "1", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), 1, in_i & !out_i & phi->atom(1)->star);
                ...
                // 1 -> 1_0, !in_i & out_i & ^phi, y := 0, x <= b, y > a

                build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1", "1_0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), 2, !in_i & out_i & phi->atom(1)->hat);


                // 1_0 -> 1_0, !in_i & out_i & ^phi, y := 0, x <= b, y <= 1

                build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1_0", "1_0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), "<= " + std::to_string(std::stoi(right->children[0]->getText()) - std::stoi(left->children[0]->getText())), 2, !in_i & out_i & phi->atom(1)->hat);


                // 1_0 -> 0, in_null & out_i & *phi, x := 0, y := 0, x > b

                build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1_0", "0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "> " : ">= ") + right->children[0]->getText(), std::string{}, 3, in_null & out_i & phi->atom(1)->star);
            ...
            *****/
            ...
            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0", "0", std::string{}, std::string{}, 1, !in_i & phi->atom(0)->star & phi->atom(1)->star & !out_i);

            // 0 -> 1, r && *p && *q, x := 0

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0", "1", std::string{}, std::string{}, 1, in_i & phi->atom(0)->star & phi->atom(1)->star & !out_i);

            // 1 -> 1, !r && ~p && *q, x < a

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "1", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), std::string{}, 0, !in_i & phi->atom(0)->tilde & phi->atom(1)->star & !out_i);

            // 1 -> 1, !r && ~p && ^q, x >= a

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "1", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), std::string{}, 0, !in_i & phi->atom(0)->tilde & phi->atom(1)->hat & !out_i);
            ...
            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "1", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), std::string{}, 1, in_i & phi->atom(0)->hat & phi->atom(1)->star & out_i);
            ...
            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "1", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), std::string{}, 1, in_i & phi->atom(0)->hat & phi->atom(1)->hat & out_i);

            // 1 -> 0, !r && ^p && *q, x := 0, x < a

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "0", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), std::string{}, 1, !in_i & phi->atom(0)->hat & phi->atom(1)->star & out_i);

            // 1 -> 0, !r && ^p && ^q, x := 0, x >= a

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "0", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), std::string{}, 1, !in_i & phi->atom(0)->hat & phi->atom(1)->hat & out_i);
