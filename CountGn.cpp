        ...
        if (phi->interval() == nullptr) {
            ...
            if (phi->existential) {
                ...
                *****/
                ...
                in_null = in_null & !bdd_ithvar(phi->id + phi->bits / 2 - 2);
                in_i = in_i | bdd_ithvar(phi->id + phi->bits / 2 - 2);
                ...
                if (out_format.has_value() && !out_flatten) {
                    ...
                    if (out_format.value()) {
                        ...
                        out_str << "location:" << name << ":ell_" << name_id_map.at("0") << "{initial: : labels: accept_" << phi->id << "_" << 0 << "}" << std::endl;

                        out_str << "location:" << name << ":ell_" << name_id_map.at("1") << "{labels: accept_" << phi->id << "_" << 0 << "}" << std::endl;
                ...
                if (!phi->weak) {
                    ...
                    // 0 -> 0, in_null & out_null & *p & *q, x := 0, y := 0
                    ...
                    // 0 -> 0, in_i & out_i & ^p & *q, x := 0, y := 0

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0", "0", std::string{}, std::string{}, 3, in_i & out_i & phi->atom(0)->hat & phi->atom(1)->star);
                ...
                if (!phi->weak) {
                    ...
                    build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0", "1", std::string{}, std::string{}, 3, in_i & out_null & phi->atom(0)->tilde & phi->atom(1)->star);

                }

                // 1 -> 1, in_null & out_null & ~p & *q, x <= b, y < a

                build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "1", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), 0, in_null & out_null & phi->atom(0)->tilde & phi->atom(1)->star);

                // 1 -> 0, in_null & out_i & *p & *q, x := 0, y := 0, x > b, y >= a

                build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "> " : ">= ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), 3, in_null & out_i & phi->atom(0)->star & phi->atom(1)->star);
            
                // 1 -> 0, in_null && out_i & ^p && *q, x := 0, y := 0, x <= b, y < a

                build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), 3, in_null & out_i & phi->atom(0)->hat & phi->atom(1)->star);

                // 1 -> 0, in_null && out_i & ^p && ^q, x := 0, y := 0, x <= b, y >= a

                build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), 3, in_null & out_i & phi->atom(0)->hat & phi->atom(1)->hat);

                // 1 -> 1_1, in_null & out_i & ~p & ^q, x <= b, y >= a

                build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "1_1", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), 0, in_null & out_i & phi->atom(0)->tilde & phi->atom(1)->hat);

                // 1_1 -> 1_1, in_null & out_i & ~p & ^q, x <= b, y >= a

                build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1_1", "1_1", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), 0, in_null & out_i & phi->atom(0)->tilde & phi->atom(1)->hat);

                // 1_1 -> 0, in_null & out_i & *p & *q, x := 0, y := 0, x > b, y >= a

                build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1_1", "0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "> " : ">= ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), 3, in_null & out_i & phi->atom(0)->star & phi->atom(1)->star);

                // 1_1 -> 0, in_null & out_i && ^p && ^q, x := 0, y := 0, x <= b, y >= a

                build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1_1", "0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), 3, in_null & out_i & phi->atom(0)->hat & phi->atom(1)->hat);
                ...
                *****/
                ...
                for (auto i = 0; i < phi->num_pairs; ++i) {
                    ...
                    // 1 -> 1, in_i & !out_i, x := 0, x <= b, y < a

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1", "1", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), 1, in_i & !out_i);
                    ...
                    // 1 -> 1_1, !in_i & out_i & ^phi, y := 0, x <= b, y >= a

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1", "1_1", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), 2, !in_i & out_i & phi->atom(1)->hat);
                    ...
                    build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1", "0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "> " : ">= ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), 3, !in_i & out_i & phi->atom(1)->star);

                    // 1_0 -> 0, !in_i & out_i & *phi, x := 0, y := 0, x > b, y >= a

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1_0", "0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "> " : ">= ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), 3, !in_i & out_i & phi->atom(1)->star);
                    ...
                    // 1_0 -> 1_1, !in_i & out_i & ^phi, y := 0, x <= b, y >= a

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1_0", "1_1", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), 2, !in_i & out_i & phi->atom(1)->hat);


                    // 1_1 -> 1_1, !in_i & out_i & ^phi, y := 0, x <= b

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1_1", "1_1", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), std::string{}, 2, !in_i & out_i & phi->atom(1)->hat);

                    // 1_1 -> 1_1, in_i & out_i & ^phi, x := 0, y := 0, x <= b

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1_1", "1_1", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), std::string{}, 3, in_i & out_i & phi->atom(1)->hat);


                    // 1_1 -> 0, in_null & out_i & *phi, x := 0, y := 0, x > b

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1_1", "0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "> " : ">= ") + right->children[0]->getText(), std::string{}, 3, in_null & out_i & phi->atom(1)->star);
                ...
                *****/
                ...
                if (!phi->weak) {
                    ...
                    build_edge(bdd_edges, name_id_map, out_str, phi->id, phi->num_pairs, "0", "0", std::string{}, std::string{}, 1, in_i & phi->atom(0)->hat & phi->atom(1)->star & out_i);
                ...
                if (!phi->weak) {
                    ...
                    build_edge(bdd_edges, name_id_map, out_str, phi->id, phi->num_pairs, "0", "1", std::string{}, std::string{}, 1, in_i & phi->atom(0)->tilde & phi->atom(1)->star & !out_i);

                }

                // 1 -> 1, ~p && *q, x < a

                build_edge(bdd_edges, name_id_map, out_str, phi->id, phi->num_pairs, "1", "1", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), std::string{}, 0, phi->atom(0)->tilde & phi->atom(1)->star & !out_i);

                // 1 -> 1, ~p && ^q, x >= a

                build_edge(bdd_edges, name_id_map, out_str, phi->id, phi->num_pairs, "1", "1", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), std::string{}, 0, phi->atom(0)->tilde & phi->atom(1)->hat & !out_i);
                ...
                if (!phi->weak) {
                    ...
                    build_edge(bdd_edges, name_id_map, out_str, phi->id, phi->num_pairs, "1", "0", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), std::string{}, 1, !in_i & phi->atom(0)->hat & phi->atom(1)->star & out_i);

                    // 1 -> 0, !r && ^p && ^q, x := 0, x >= a

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, phi->num_pairs, "1", "0", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), std::string{}, 1, !in_i & phi->atom(0)->hat & phi->atom(1)->hat & out_i);

                } else {

                    // 1 -> 0, ^p && *q, x := 0, x < a

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, phi->num_pairs, "1", "0", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), std::string{}, 1, phi->atom(0)->hat & phi->atom(1)->star & out_i);

                    // 1 -> 0, ^p && ^q, x := 0, x >= a

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, phi->num_pairs, "1", "0", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), std::string{}, 1, phi->atom(0)->hat & phi->atom(1)->hat & out_i);
