...
namespace mightypplcpp {
    ...
    std::any MitlAtomNumberingVisitor::visitAtomCFn(MitlParser::AtomCFnContext *ctx) {
        ...
        ctx->num_pairs = std::floor(std::stoi(left->children[0]->getText()) / (std::stoi(right->children[0]->getText()) - std::stoi(left->children[0]->getText()))) + (std::stoi(left->children[0]->getText()) % (std::stoi(right->children[0]->getText()) - std::stoi(left->children[0]->getText())) == 0 && left_delim->getSymbol()->getType() == MitlParser::LParen ? 2 : 1);
