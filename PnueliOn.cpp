...
namespace mightypplcpp {
    ...
    std::pair<std::vector<monitaal::TAwithBDDEdges>, std::string> build_pnuelion(const MitlParser::AtomContext* phi_) {
                ...
                if (phi->existential) {
                    ...
                    *****/
                    ...
                    for (auto j = 0; j < phi->atoms.size(); ++j) {
                        ...
                        // 1_j -> 1_j, out_null & in_null & ~p_j (*p_j), x <= a

                        build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1_" + std::to_string(j), "1_" + std::to_string(j), (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), std::string{}, 0, (j == 0 ? out_null & in_null & phi->atoms[j]->star : out_null & in_null & phi->atoms[j]->tilde));

                        // 1_j+1 (0) -> 1_j, out_null && in_null (in_i) & ^p_j (x := 0), x <= a ()

                        build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, (j + 1 == phi->atoms.size() ? "0" : "1_" + std::to_string(j + 1)), "1_" + std::to_string(j), (j + 1 == phi->atoms.size() ? std::string{} : (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText()), std::string{}, (j + 1 == phi->atoms.size() ? 1 : 0), out_null & (j + 1 == phi->atoms.size() ? in_i : in_null) & phi->atoms[j]->hat);
