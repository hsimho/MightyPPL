                ...
                if (phi->existential) {
                    ...
                    *****/
                    ...
                    if (out_format.has_value() && !out_flatten) {
                        ...
                        if (out_format.value()) {
                            ...
                            for (auto j = 0; j < phi->atoms.size(); ++j) {
                                out_str << "location:" << name << ":ell_" << name_id_map.at("1_" + std::to_string(j)) << "{}" << std::endl;
                            }
                    ...
                    // 0 -> 0, in_null & out_null, x := 0

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0", "0", std::string{}, std::string{}, 1, in_null & out_null);

                    // 0 -> 1_0, in_i & out_null, x := 0

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0", "1_0", std::string{}, std::string{}, 1, in_i & out_null);

                    for (auto j = 0; j < phi->atoms.size(); ++j) {

                        // 1_j -> 1_j, in_null & out_null & ~p_j, x <= a

                        build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1_" + std::to_string(j), "1_" + std::to_string(j), (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), std::string{}, 0, in_null & out_null & phi->atoms[j]->tilde);

                        // 1_j -> 1_j+1 (0), in_null & out_null (out_i) & ^p_j, (x := 0), x <= a

                        build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1_" + std::to_string(j), (j + 1 == phi->atoms.size() ? "0" : "1_" + std::to_string(j + 1)), (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), std::string{}, (j + 1 == phi->atoms.size() ? 1 : 0), in_null & (j + 1 == phi->atoms.size() ? out_i : out_null) & phi->atoms[j]->hat);
                    ...
                    *****/
                    ...
                    for (auto i = 0; i < phi->atoms.size(); ++i) {
                        ...
                        // acc_map.insert({ name, { {0}, {0, 1 + phi->atoms.size()} } });
                        ...
                        for (auto j = 0; j < phi->atoms.size(); ++j) {
                            ...
                            // 1_j -> 1_j, ((in_i | in_null) & *p_j) !in_i && ~p_j, x <= a

                            build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1_" + std::to_string(j), "1_" + std::to_string(j), (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), std::string{}, 0, (j == 0 ? (in_i | in_null) & phi->atoms[0]->star & !out_i : !in_i & phi->atoms[j]->tilde & !out_i));

                            // 1_j -> 1_j+1 (0), !in_i && ^p_j (&& out_i), (x := 0), x <= a

                            build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1_" + std::to_string(j), (j + 1 == phi->atoms.size() ? "0" : "1_" + std::to_string(j + 1)), (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), std::string{}, (j + 1 == phi->atoms.size() ? 1 : 0), !in_i & phi->atoms[j]->hat & (j + 1 == phi->atoms.size() ? out_i : !out_i));

                        }

                        // 1_n-1 -> 2, in_i && ^p_j, x := 0, x <= a

                        build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1_" + std::to_string(phi->atoms.size() - 1), "2", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), std::string{}, 1, in_i & phi->atoms[phi->atoms.size() - 1]->hat & out_i);

                        // 2 -> 1_1, !in_i && ^p_j, x <= a

                        build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "2", "1_1", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), std::string{}, 0, !in_i & phi->atoms[0]->hat & !out_i);

                        // 2 -> 1_0, (in_i | in_null) & *p_j, x <= a

                        build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "2", "1_0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), std::string{}, 0, (in_i | in_null) & phi->atoms[0]->star & !out_i);
