        ...
        if (phi->interval() == nullptr) {
            ...
            *****/
            ...
            for (auto i = 0; i < phi->num_pairs; ++i) {
                ...
                build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1", "1", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "> " : ">= ") + right->children[0]->getText(), std::string{}, 0, out_i & !in_i & phi->atom(1)->star);


                // 0 -> 1, !out_i & in_i & *phi, x := 0, y := 0

                build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "0", "1", std::string{}, std::string{}, 3, !out_i & in_i & phi->atom(1)->star);


                // 1_0 -> 1, !out_i & in_i & ^phi, y := 0, x <= b

                build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1_0", "1", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), std::string{}, 2, !out_i & in_i & phi->atom(1)->hat);


                // 1_0 -> 1_0, !out_i & in_i & ^phi, y := 0, x <= b

                build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1_0", "1_0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), std::string{}, 2, !out_i & in_i & phi->atom(1)->hat);


                // 0 -> 1_0, out_null & in_i & *phi, x := 0, y := 0

                build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "0", "1_0", std::string{}, std::string{}, 3, out_null & in_i & phi->atom(1)->star);
            ...
            *****/
            ...
            if (out_format.has_value() && out_format.value() && !out_flatten) {
                ...
                out_str << "location:" << "TA_" << phi->id << "_" << phi->num_pairs << ":ell_0i{initial: }" << std::endl;
                out_str << "location:" << "TA_" << phi->id << "_" << phi->num_pairs << ":ell_0{labels: accept_" << phi->id << "_" << phi->num_pairs << "}" << std::endl;
                out_str << "location:" << "TA_" << phi->id << "_" << phi->num_pairs << ":ell_1{labels: accept_" << phi->id << "_" << phi->num_pairs << "}" << std::endl;
                out_str << "location:" << "TA_" << phi->id << "_" << phi->num_pairs << ":ell_2{labels: accept_" << phi->id << "_" << phi->num_pairs << "}" << std::endl;
            ...
            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0i", "0", std::string{}, std::string{}, 1, !out_i & phi->atom(0)->star & phi->atom(1)->star & !in_i);

            // 0i -> 0, r && *p && *q, x := 0, x >= a

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0i", "0", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), std::string{}, 1, out_i & phi->atom(0)->star & phi->atom(1)->star & !in_i);
            ...
            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0i", "1", std::string{}, std::string{}, 0, phi->atom(0)->tilde & phi->atom(1)->hat & !in_i);

            // 0i -> 2, ~p && *q, x < a

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0i", "2", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), std::string{}, 0, phi->atom(0)->tilde & phi->atom(1)->star & !in_i);

            // 0i -> 2, ~p && *q, x := 0

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0i", "2", std::string{}, std::string{}, 1, phi->atom(0)->tilde & phi->atom(1)->star & !in_i);

            // 0i -> 2, r && ^p && *q, x := 0, x >= a

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0i", "2", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), std::string{}, 1, out_i & phi->atom(0)->hat & phi->atom(1)->star & in_i);
            ...
            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0i", "1", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), std::string{}, 1, out_i & phi->atom(0)->hat & phi->atom(1)->hat & in_i);
            ...
            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0i", "2", std::string{}, std::string{}, 1, !out_i & phi->atom(0)->hat & phi->atom(1)->star & in_i);

            // 0i -> 1, !r && ^p && ^q, x := 0

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0i", "1", std::string{}, std::string{}, 1, !out_i & phi->atom(0)->hat & phi->atom(1)->hat & in_i);
            ...
            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "2", "0", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), std::string{}, 1, out_i & phi->atom(0)->star & phi->atom(1)->star & !in_i);

            // 1 -> 1, ~p && ^q

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "1", std::string{}, std::string{}, 0, phi->atom(0)->tilde & phi->atom(1)->hat & !in_i);

            // 2 -> 2, ~p && *q, x < a

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "2", "2", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), std::string{}, 0, phi->atom(0)->tilde & phi->atom(1)->star & !in_i);

            // 1 -> 2, ~p && *q, x := 0

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "2", std::string{}, std::string{}, 1, phi->atom(0)->tilde & phi->atom(1)->star & !in_i);

            // 1 -> 2, r && ^p && *q, x := 0, x >= a

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "2", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), std::string{}, 1, out_i & phi->atom(0)->hat & phi->atom(1)->star & in_i);

            // 2 -> 2, r && ^p && *q, x := 0, x < a

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "2", "2", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), std::string{}, 1, out_i & phi->atom(0)->hat & phi->atom(1)->star & in_i);

            // 1 -> 1, r && ^p && ^q, x := 0, x >= a

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "1", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), std::string{}, 1, out_i & phi->atom(0)->hat & phi->atom(1)->hat & in_i);

            // 2 -> 1, r && ^p && ^q, x := 0, x < a

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "2", "1", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), std::string{}, 1, out_i & phi->atom(0)->hat & phi->atom(1)->hat & in_i);

            // 0 -> 2, !r && ^p && *q, x := 0

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0", "2", std::string{}, std::string{}, 1, !out_i & phi->atom(0)->hat & phi->atom(1)->star & in_i);

            // 0 -> 1, !r && ^p && ^q, x := 0

            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0", "1", std::string{}, std::string{}, 1, !out_i & phi->atom(0)->hat & phi->atom(1)->hat & in_i);



            components.push_back(monitaal::TAwithBDDEdges(name, clocks, locations, bdd_edges, 3));
            locations.clear();
            name_id_map.clear();
            bdd_edges.clear();
