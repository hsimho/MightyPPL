        ...
        if (phi->interval() == nullptr) {
            ...
            *****/
            ...
            if (!phi->weak) {
                ...
                build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0", "1", std::string{}, std::string{}, 0, bdd_ithvar(phi->id) & phi->atom(0)->tilde & phi->atom(1)->hat);
                ...
                build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "0", std::string{}, std::string{}, 0, phi->atom(0)->hat & phi->atom(1)->hat);
            ...
            if (left_delim->getSymbol()->getType() == MitlParser::LBrack && left->children[0]->getText() == "0") {
                ...
                *****/
                ...
                if (!phi->weak) {
                    ...
                    build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0", "1", std::string{}, std::string{}, 1, bdd_ithvar(phi->id) & phi->atom(0)->tilde & phi->atom(1)->hat);
                    ...
                    build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "1", std::string{}, std::string{}, 1, bdd_ithvar(phi->id) & phi->atom(0)->tilde & phi->atom(1)->hat);

                    // 1 -> 0, ^p && ^q, x := 0, x <= a

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), std::string{}, 1, phi->atom(0)->hat & phi->atom(1)->hat);
                ...
                *****/
                ...
                if (!phi->weak) {
                    ...
                    build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0", "1", std::string{}, std::string{}, 1, bdd_ithvar(phi->id) & phi->atom(0)->tilde & phi->atom(1)->star);

                }

                // 1 -> 1, ~p && *q, x < a

                build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "1", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), std::string{}, 0, phi->atom(0)->tilde & phi->atom(1)->star);

                // 1 -> 1, ~p && ^q, x >= a

                build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "1", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), std::string{}, 0, phi->atom(0)->tilde & phi->atom(1)->hat);
                ...
                if (!phi->weak) {
                    ...
                    build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "0", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), std::string{}, 1, phi->atom(0)->hat & phi->atom(1)->star);

                    // 1 -> 0, ^p && ^q, x := 0, x >= a

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "0", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), std::string{}, 1, phi->atom(0)->hat & phi->atom(1)->hat);
