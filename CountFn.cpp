        ...
        if (phi->interval() == nullptr) {
            ...
            if (phi->existential) {
                ...
                *****/
                ...
                build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "1", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), std::string{}, 0, in_null & out_null & phi->atom(0)->hat & phi->atom(1)->star);

                // 1 -> 0, in_null & out_i & *p & ^q, x := 0, y := 0, x <= b, y >= a

                build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1", "0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), (left_delim->getSymbol()->getType() == MitlParser::LBrack ? ">= " : "> ") + left->children[0]->getText(), 3, in_null & out_i & phi->atom(0)->star & phi->atom(1)->hat);
     ...
     //           name_id_map.insert({"2", 2});
                ...
                if (out_format.has_value() && !out_flatten) {
                    ...
                    if (out_format.value()) {
                        ...
                        // out_str << "location:" << name << ":ell_" << name_id_map.at("2") << "{" << (out_fin ? "" : "labels: accept_" + std::to_string(phi->id) + "_" + std::to_string(phi->num_pairs)) << "}" << std::endl;
                ...
                build_edge(bdd_edges, name_id_map, out_str, phi->id, phi->num_pairs, "1", "1", (left_delim->getSymbol()->getType() == MitlParser::LBrack ? "< " : "<= ") + left->children[0]->getText(), std::string{}, 0, !in_i & phi->atom(0)->hat & bdd_true() & !out_i);
                ...
                build_edge(bdd_edges, name_id_map, out_str, phi->id, phi->num_pairs, "1", "1", std::string{}, std::string{}, 0, !in_i & phi->atom(0)->hat & phi->atom(1)->hat & out_i);

                // 1 -> 1, r && ^p && ^q, x := 0

                build_edge(bdd_edges, name_id_map, out_str, phi->id, phi->num_pairs, "1", "1", std::string{}, std::string{}, 1, in_i & phi->atom(0)->hat & phi->atom(1)->hat & out_i);
