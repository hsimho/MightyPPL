        ...
        if (phi->interval() == nullptr) {
            ...
            if (left_delim->getSymbol()->getType() == MitlParser::LBrack && left->children[0]->getText() == "0") {
                ...
                if (phi->existential) {
                    ...
                    *****/
                    ...
                    for (auto j = 0; j < phi->atoms.size(); ++j) {
                        ...
                        build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1_" + std::to_string(j), "1_" + std::to_string(j), (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), std::string{}, 0, in_null & out_null & phi->atoms[j]->hat);

                        if (j + 1 != phi->atoms.size()) {

                            // 1_j -> 1_j+1, in_null & out_null & ~p_j, x <= a

                            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1_" + std::to_string(j), "1_" + std::to_string(j + 1), (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "<= " : "< ") + right->children[0]->getText(), std::string{}, 0, in_null & out_null & phi->atoms[j]->tilde);

                        }

                        // 1_j -> 0, in_null & out_i & *p_j, x := 0, x > a

                        build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1_" + std::to_string(j), "0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "> " : ">= ") + right->children[0]->getText(), std::string{}, 1, in_null & out_i & phi->atoms[j]->star);
                ...
                if (!phi->existential) {
                    ...
                    //     for (int i = 0; i < phi->atoms.size(); ++i) {
                    //         s.insert(s.end(), i);
                    //         s.insert(s.end(), phi->atoms.size() + i);
                    //     }
                    //     acc_map.insert({ name, { s, s } });
                    // }
