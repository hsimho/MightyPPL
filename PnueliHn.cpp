...
namespace mightypplcpp {
    ...
    std::pair<std::vector<monitaal::TAwithBDDEdges>, std::string> build_pnuelihn(const MitlParser::AtomContext* phi_) {
        ...
        if (phi->interval() == nullptr) {
            ...
            std::cout << "in = " << "p_" << phi->id << " to " << "p" << phi->id + phi->bits / 2 - 2 << std::endl;
            std::cout << "out = " << "p_" << phi->id + phi->bits / 2 << " to " << "p" << phi->id + phi->bits - 2 << std::endl;
            ...
            if (left_delim->getSymbol()->getType() == MitlParser::LBrack && left->children[0]->getText() == "0") {
                ...
                if (phi->existential) {
                    ...
                    *****/
                    ...
                    in_i = encode(0 + 1, phi->id, phi->bits / 2 - 1);
                    out_i = encode(0 + 1, phi->id + phi->bits / 2, phi->bits / 2 - 1);
                    ...
                    for (auto j = 0; j < phi->atoms.size(); ++j) {
                        ...
                        locations.push_back(monitaal::location_t(true, 1 + j + 1 + phi->atoms.size(), "s1i_" + std::to_string(j), empty_invariant));
                        name_id_map.insert({"1i_" + std::to_string(j), 1 + j + 1 + phi->atoms.size()});
                    ...
                    if (out_format.has_value() && !out_flatten) {
                        ...
                        if (out_format.value()) {
                            ...
                            for (auto j = 0; j < phi->atoms.size(); ++j) {
                                out_str << "location:" << name << ":ell_" << name_id_map.at("1_" + std::to_string(j)) << "{labels: accept_" << phi->id << "_" << 0 << "}" << std::endl;
                                out_str << "location:" << name << ":ell_" << name_id_map.at("1i_" + std::to_string(j)) << "{labels: accept_" << phi->id << "_" << 0 << "}" << std::endl;
                            }
                    ...
                    // 0i -> 0, out_i & in_null, x := 0

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0i", "0", std::string{}, std::string{}, 1, out_i & in_null);

                    // 0i -> 0, out_null & in_null, x := 0

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0i", "0", std::string{}, std::string{}, 1, out_null & in_null);

                    for (auto j = 0; j < phi->atoms.size(); ++j) {


                        // 0i -> 1i_j, out_null & in_i & ^p_j, x := 0

                        build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0i", "1i_" + std::to_string(j), std::string{}, std::string{}, 1, out_null & in_i & phi->atoms[j]->hat);

                        if (j + 1 != phi->atoms.size()) {

                            // 0i -> 1i_j, out_null & in_i & ~p_j, x := 0

                            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0i", "1i_" + std::to_string(j), std::string{}, std::string{}, 1, out_null & in_i & phi->atoms[j]->tilde);

                        }

                        // 0i -> 1_j, out_null & in_i & *p_j, x := 0

                        build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0", "1_" + std::to_string(j), std::string{}, std::string{}, 1, out_null & in_i & phi->atoms[j]->star);
                        
                    }



                    // 0 -> 0, out_null & in_null, x := 0

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "0", "0", std::string{}, std::string{}, 1, out_null & in_null);

                    // 1_0 -> 0, out_i & in_null, x := 0, x > a

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1_0", "0", (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "> " : ">= ") + right->children[0]->getText(), std::string{}, 1, out_i & in_null);

                    
                    // 1i_0 -> 0, out_i & in_null, x := 0

                    build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1i_0", "0", std::string{}, std::string{}, 1, out_i & in_null);


                    for (auto j = 0; j < phi->atoms.size(); ++j) {

                        // 1_j -> 1_j, out_null & in_null & ^p_j

                        build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1_" + std::to_string(j), "1_" + std::to_string(j), std::string{}, std::string{}, 0, out_null & in_null & phi->atoms[j]->hat);

                        // 1i_j -> 1i_j, out_null & in_null & ^p_j

                        build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1i_" + std::to_string(j), "1i_" + std::to_string(j), std::string{}, std::string{}, 0, out_null & in_null & phi->atoms[j]->hat);


                        if (j + 1 != phi->atoms.size()) {

                            // 1_j+1 -> 1_j, out_null & in_null & ~p_j

                            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1_" + std::to_string(j + 1), "1_" + std::to_string(j), std::string{}, std::string{}, 0, out_null & in_null & phi->atoms[j]->tilde);

                            // 1i_j+1 -> 1i_j, out_null & in_null & ~p_j

                            build_edge(bdd_edges, name_id_map, out_str, phi->id, 0, "1i_" + std::to_string(j + 1), "1i_" + std::to_string(j), std::string{}, std::string{}, 0, out_null & in_null & phi->atoms[j]->tilde);

                        }

                        // 0 -> 1_j, out_null & in_i & *p_j, x := 0
                    ...
                    *****/
                    ...
                    for (auto i = 0; i < phi->atoms.size(); ++i) {
                        ...
                        if (out_format.has_value() && !out_flatten) {
                            ...
                            if (out_format.value()) {
                                ...
                                for (auto j = 0; j < phi->atoms.size(); ++j) {
                                    ...
                                    if (i == 0) {
                                        out_str << "location:" << name << ":ell_" << name_id_map.at("1i_" + std::to_string(j)) << "{labels: accept_" << phi->id << "_" << i << "}" << std::endl;
                                    }
                                }
                        ...
                        if (i == 0) {
                            ...
                            build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "0i", "0", std::string{}, std::string{}, 1, !in_i);

                            for (auto j = 0; j < phi->atoms.size(); ++j) {


                                // 0i -> 1i_j, ^p_j & in_i, x := 0

                                build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "0i", "1i_" + std::to_string(j), std::string{}, std::string{}, 1, phi->atoms[j]->hat & in_i);

                                if (j + 1 != phi->atoms.size()) {

                                    // 0i -> 1i_j, ~p_j & in_i, x := 0

                                    build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "0i", "1i_" + std::to_string(j), std::string{}, std::string{}, 1, phi->atoms[j]->tilde & in_i);

                                }

                                // 0i -> 1_j, *p_j & in_i, x := 0

                                build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "0", "1_" + std::to_string(j), std::string{}, std::string{}, 1, phi->atoms[j]->star & in_i);
                        ...
                        if (i == 0) {
                            ...
                            build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1i_0", "0", std::string{}, std::string{}, 1, out_i & !in_i);
                        ...
                        for (auto j = 0; j < phi->atoms.size(); ++j) {
                            ...
                            if (i == 0) {
                                ...
                                // 1i_j -> 1i_j, out_null && ^p_j

                                build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1i_" + std::to_string(j), "1i_" + std::to_string(j), std::string{}, std::string{}, 0, out_null & phi->atoms[j]->hat & !in_i);

                                // 1i_j -> 1i_j, out_i && ^p_j

                                build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1i_" + std::to_string(j), "1i_" + std::to_string(j), (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "> " : ">= ") + right->children[0]->getText(), std::string{}, 0, out_i & phi->atoms[j]->hat & !in_i);
                            ...
                            if (j + 1 != phi->atoms.size()) {
                                ...
                                if (i == 0) {
                                    ...
                                    // 1i_j+1 -> 1i_j, out_null && ~p_j

                                    build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1i_" + std::to_string(j + 1), "1i_" + std::to_string(j), std::string{}, std::string{}, 0, out_null & phi->atoms[j]->tilde & !in_i);

                                    // 1i_j+1 -> 1i_j, out_i && ~p_j

                                    build_edge(bdd_edges, name_id_map, out_str, phi->id, i, "1i_" + std::to_string(j + 1), "1i_" + std::to_string(j), (right_delim->getSymbol()->getType() == MitlParser::RBrack ? "> " : ">= ") + right->children[0]->getText(), std::string{}, 0, out_i & phi->atoms[j]->tilde & !in_i);
                        ...
                        ++components_counter;
                        components.push_back(monitaal::TAwithBDDEdges(name, clocks, locations, bdd_edges, name_id_map.at(i == 0 ? "0i" : "0")));
                        locations.clear();
                        name_id_map.clear();
                        bdd_edges.clear();
                ...
                if (!phi->existential) {
                    ...
                    locations.push_back(monitaal::location_t(false, 2 * phi->atoms.size(), "s0i", empty_invariant));
                    name_id_map.insert({"0i", 2 * phi->atoms.size()});
                    ...
                    if (out_format.has_value() && !out_flatten) {
                        ...
                        if (out_format.value()) {
                            ...
                            for (int i = 0; i < phi->atoms.size(); ++i) {
                                out_str << "location:" << name << ":ell_" << name_id_map.at("e_" + std::to_string(i)) << "{" << "" << "labels: accept_seq_in_" << phi->id << "}" << std::endl;
                            }
                    ...
                    for (int i = 0; i < phi->atoms.size(); ++i) {
                        ...
                        if (i == phi->atoms.size() - 1) {
                            ...
                            build_untimed_edge(bdd_edges, name_id_map, out_str, "seq_in_" + std::to_string(phi->id), "0i", "e_" + std::to_string(i), in_null);
                            build_untimed_edge(bdd_edges, name_id_map, out_str, "seq_in_" + std::to_string(phi->id), "0i", "h_" + std::to_string(i + 1 == phi->atoms.size() ? 0 : i + 1), in_next_i);
                    ...
                    ++components_counter;
                    components.push_back(monitaal::TAwithBDDEdges("seq_in_" + std::to_string(phi->id), clocks, locations, bdd_edges, name_id_map.at("0i")));
                    locations.clear();
                    name_id_map.clear();
                    bdd_edges.clear();
